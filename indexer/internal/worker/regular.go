...
func (rw *RegularWorker) determineStartingBlock() uint64 {
	...
	offset := uint64(rw.config.StartOffset)
	...
	if err2 != nil || kvLatest == 0 || rw.config.FromLatest {
		if chainLatest > offset {
			return chainLatest - offset
		}
		return chainLatest
	}
...
func splitCatchupRange(r blockstore.CatchupRange, maxSize uint64) []blockstore.CatchupRange {
	...
	if maxSize <= 0 || totalBlocks <= maxSize {
		return []blockstore.CatchupRange{r}
	}
